...
begin
    ...
    then
		insert into usuarios(Username, Pass, Nombre, Email, Fnacimiento) values(Usuario, Pass, nombre, email, fNacimiento);
        set Registrado = (1) ;
        set Mensaje = ('Usuario no existente, se ha registrado con exito');
    else
		set Registrado = (0);
...
IN Usuarios varchar(100),
IN Passwords varchar (100)
)
begin
	if(exists(select * from usuarios where Username = Usuarios and Pass = Passwords))
    then
		select idUsuarios from usuarios where Username = Usuarios and Pass = Passwords;
	else
		select ('0');
	end if;
end ;;
